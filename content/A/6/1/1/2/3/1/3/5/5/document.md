---
id: 02deeacc-5305-4a08-a5aa-2aabeb5591be
docNo: A.6.1.1.2.3.1.3.5.5
name: Term-End Automatic Offboarding
type: Core
depth: 10
childType: sections_and_primary_docs
---

###### A.6.1.1.2.3.1.3.5.5 - Term-End Automatic Offboarding [Core]

At the end of a Delegate's six (6) month term (see [A.6.1.1.2.3.1.3.4.3 - Minimum Term](c612d4e4-96c4-4ccf-a830-7f742338cfd9)), if the Spark Foundation has not recorded a re-approval, the Delegate is automatically offboarded effective 00:00 UTC on the day after term end. The Operational Facilitator updates the Registry and posts a notice on the Sky Forum.
